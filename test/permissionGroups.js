...
contract('PermissionGroups', function(accounts) {
    ...
    it("should test quick admin transfer rejected for address 0.", async function () {
        try {
            await permissionsInst.transferAdminQuickly('0x0000000000000000000000000000000000000000', {from: mainAdmin});
            assert(false, "throw was expected in line above.")
        }
        catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
    ...
    it("should test new admin address 0 is reverted.", async function () {
        ...
        try {
            await permissionsInst.transferAdmin('0x0000000000000000000000000000000000000000');
            assert(false, "throw was expected in line above.")
        }
        catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
    ...
    it("should test can't add more then MAX_GROUP_SIZE (50) operators.", async function () {
        ...
        for (let i = (operators.length); i < 50; i++) {
            let prefix = '0x00000000000000000000000000000000000000';
            if (i < 10) {
              prefix += '0' + i;
            } else {
              prefix += i;
            }
            await permissionsInst.addOperator(prefix);
        }

        try {
            await permissionsInst.addOperator('0x0000000000000000000000000000000000000100');
            assert(false, "throw was expected in line above.")
        }
        catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
    ...
    it("should test can't add more then MAX_GROUP_SIZE (50) alerters.", async function () {
        ...
        for (let i = (alerters.length); i < 50; i++) {
          ...
          if (i < 10) {
            ...
            await permissionsInst.addAlerter(prefix);
        }

        try {
            await permissionsInst.addAlerter('0x0000000000000000000000000000000000000100');
            assert(false, "throw was expected in line above.")
        }
        catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
