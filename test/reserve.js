...
//        console.log("current block: " + currentBlock);
    ...
    it("should init reserve and set all reserve data including balances", async function () {
        ...
        await reserveInst.setContracts(network, convRatesInst.address, '0x0000000000000000000000000000000000000000');
    ...
    it("should perform small buy (no steps) and check: balances changed, rate is expected rate.", async function () {
        ...
        //perform trade
        await reserveInst.trade(ethAddress, amountWei, tokenAdd[tokenInd], user1, buyRate, 0, true, {from:network, value:amountWei});
...
//            console.log("expected from function" + expected);
    ...
    it("should perform small sell and check: balances changed, rate is expected rate.", async function () {
        ...
        //perform trade
        await reserveInst.trade(tokenAdd[tokenInd], amountTwei, ethAddress, user2, sellRate, 0, true, {from:network});
    ...
    it("should verify trade success when validation disabled.", async function () {
        ...
        //perform trade
        await reserveInst.trade(tokenAdd[tokenInd], amountTwei, ethAddress, user2, sellRate, 0, false, {from:network});
    ...
    it("should test sell trade reverted without token approved.", async function () {
        ...
        //
        try {
            await reserveInst.trade(tokenAdd[tokenInd], amount, ethAddress, user2, sellRate, 0, true, {from:network});
    ...
    it("should test trade reverted when conversion rate 0.", async function () {
        ...
        //
        try {
            await reserveInst.trade(tokenAdd[tokenInd], amount, ethAddress, user2, 0, 0, true, {from:network});
            assert(false, "throw was expected in line above.")
        } catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
    ...
    it("should test trade reverted when dest amount is 0.", async function () {
        ...
        //
        try {
            await reserveInst.trade(tokenAdd[tokenInd], amountLow, ethAddress, user2, sellRate, 0, true, {from:network});
            assert(false, "throw was expected in line above.")
        } catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }

        await reserveInst.trade(tokenAdd[tokenInd], amountHigh, ethAddress, user2, sellRate, 0, true, {from:network});
    ...
    it("should test buy trade reverted when not sending correct ether value.", async function () {
       ...
       //test trade reverted when sending wrong ether value
        try {
            await reserveInst.trade(ethAddress, amount, tokenAdd[tokenInd], user2, rate, 0, true, {from:network, value:1});
            assert(false, "throw was expected in line above.")
        } catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
        ...
        await reserveInst.trade(ethAddress, amount, tokenAdd[tokenInd], user2, rate, 0, true, {from:network, value:amount});
    });
    ...
    it("should test trade reverted when not sent from network.", async function () {
       ...
       //test trade reverted when sending wrong ether value
        try {
            await reserveInst.trade(ethAddress, amount, tokenAdd[tokenInd], user2, rate, 0, true, {from:operator, value:amount});
            assert(false, "throw was expected in line above.")
        } catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
    ...
    it("should test trade reverted when sending ether value with sell trade.", async function () {
       ...
       //
       try {
           await reserveInst.trade(tokenAdd[tokenInd], amount, ethAddress, user2, sellRate, 0, true, {from:network, value:3});
           assert(false, "throw was expected in line above.")
       } catch(e){
           assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
       }

       await reserveInst.trade(tokenAdd[tokenInd], amount, ethAddress, user2, sellRate, 0, true, {from:network, value: 0});
    ...
    it("should revert when not enough balance for fee", async function() {
        let tokenInd = 1;
        ...
        // fee is all balance
        try {
          await reserveInst.trade(tokenAdd[tokenInd], amount, ethAddress, user2, sellRate, ethBalance, true, {from:network, value: 0});
          assert(false, "throw was expected in line above.")
        } catch(e) {
          assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
        let expectedDestAmount = await reserveInst.getDestQty(tokenAdd[tokenInd], ethAddress, amount, sellRate);
        feeInWei = ethBalance - expectedDestAmount + 1;
        // fee is greater than remaining balance
        try {
          await reserveInst.trade(tokenAdd[tokenInd], amount, ethAddress, user2, sellRate, feeInWei, true, {from:network, value: 0});
          assert(false, "throw was expected in line above.")
        } catch(e) {
          assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
        ...
        let newETHBalance = await Helper.getBalancePromise(reserveInst.address);
        assert.equal(newETHBalance, ethBalance - expectedDestAmount - feeInWei, "Fee is not transferred from reserve");
    ...
    it("should test reverted scenario for set contracts call.", async function () {
        //legal call
        await reserveInst.setContracts(network, convRatesInst.address, '0x0000000000000000000000000000000000000000', {from:admin});

        try {
            await reserveInst.setContracts('0x0000000000000000000000000000000000000000', convRatesInst.address, '0x0000000000000000000000000000000000000000', {from:admin});
            assert(false, "throw was expected in line above.")
        } catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
        ...
        try {
            await reserveInst.setContracts(network, '0x0000000000000000000000000000000000000000', '0x0000000000000000000000000000000000000000', {from:admin});
            assert(false, "throw was expected in line above.")
        } catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
    ...
    it("should test get src qty when dst qty is very small and source is near 0.", async function () {
        ...
        let dstQty = 200;
        let rate = new BigNumber(precisionUnits / 2);

        //make sure src qty rounded up.
        let getSrcQTY = await reserveInst.getSrcQty(token3Dec.address, ethAddress, dstQty, '0x' + rate.toString(16));
        ...
        getSrcQTY = await reserveInst.getSrcQty(ethAddress, token3Dec.address, dstQty, '0x' + rate.toString(16));
    ...
    it("should test get src qty reverted when decimals diff > max decimals diff (18).", async function () {
        ...
        try {
           getSrcQTY = await reserveInst.getSrcQty(token30Dec.address, token3Dec.address, dstQty, '0x' + rate.toString(16));
           assert(false, "throw was expected in line above.")
        } catch(e){
           assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
        ...
        try {
           getSrcQTY = await reserveInst.getSrcQty(token3Dec.address, token30Dec.address, dstQty, '0x' + rate.toString(16));
           assert(false, "throw was expected in line above.")
        } catch(e){
           assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
    ...
    it("should revert when not enough balance for fee when swapping eth to token", async function() {
      let tokenInd = 1;
      let amountTwei = maxPerBlockImbalance - 1;
      let token = tokens[tokenInd];
      let amount = 10;
      ...
      let ethBalance = await Helper.getBalancePromise(reserveInst.address);
      let buyRate = await reserveInst.getConversionRate(ethAddress, tokenAdd[tokenInd], amount, currentBlock);
      feeInWei = (new BigNumber(ethBalance)).toNumber() + amount + 1;
      // fee is greater than balance + new amount
      try {
        await reserveInst.trade(ethAddress, amount, tokenAdd[tokenInd], user2, buyRate, feeInWei, true, {from:network, value: amount});
        assert(false, "throw was expected in line above.")
      } catch(e) {
        assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
      }
      feeInWei = (new BigNumber(ethBalance)).toNumber() + amount;
      // should still success as balance is added amount of eth
      await reserveInst.trade(ethAddress, amount, tokenAdd[tokenInd], user2, buyRate, feeInWei, true, {from:network, value: amount});

      let newETHBalance = await Helper.getBalancePromise(reserveInst.address);
      assert.equal(newETHBalance.valueOf(0), 0, "Balance should be zero");
    });
    ...
    it("should zero reserve balance and see that get rate returns zero when not enough dest balance", async function() {
        ...
        assert.notEqual(tokenBal.valueOf(), 0, "token balance is not 0");
        ...
        await reserveInst.approveWithdrawAddress(tokenAdd[tokenInd], withDrawAddress, true);
        await reserveInst.withdraw(tokenAdd[tokenInd], tokenBal, withDrawAddress, {from: operator});
        tokenBal = await token.balanceOf(reserveInst.address);
        assert.equal(tokenBal.valueOf(0), 0, "expected balance 0");
    ...
    it("should test can't init this contract with empty contracts (address 0).", async function () {
        ...
        try {
           reserve = await Reserve.new(network, convRatesInst.address, '0x0000000000000000000000000000000000000000');
           assert(false, "throw was expected in line above.")
        } catch(e){
           assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
        ...
        try {
           reserve =  await Reserve.new(network, '0x0000000000000000000000000000000000000000', admin);
           assert(false, "throw was expected in line above.")
        } catch(e){
           assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
        ...
        try {
           reserve =  await Reserve.new('0x0000000000000000000000000000000000000000', convRatesInst.address, admin);
           assert(false, "throw was expected in line above.")
        } catch(e){
           assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
        ...
        try {
           await reserve.setContracts('0x0000000000000000000000000000000000000000', convRatesInst.address, '0x0000000000000000000000000000000000000000');
           assert(false, "throw was expected in line above.")
        } catch(e){
           assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
        ...
        try {
           await reserve.setContracts(network, '0x0000000000000000000000000000000000000000', '0x0000000000000000000000000000000000000000');
           assert(false, "throw was expected in line above.")
        } catch(e){
           assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
        ...
        await reserve.setContracts(network, convRatesInst.address, '0x0000000000000000000000000000000000000000');
    });
});
