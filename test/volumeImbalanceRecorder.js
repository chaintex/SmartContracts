...
contract('VolumeImbalanceRecorder', function(accounts) {
    ...
    it("should test encode / decode of token imbalance data.", async function() {
        ...
        // console.log("Start int: " + startInt.toString(16));
        let toStruct = await imbalanceInst.callDecodeTokenImbalanceData("0x" + startInt.toString(16));
        let toInt = await imbalanceInst.callEncodeTokenImbalanceData(toStruct[0], toStruct[1], toStruct[2], toStruct[3]);
        toInt = new BigNumber(toInt);
        // console.log("To int: " + toInt.toString(16));

        assert.equal(startInt.toString(16), toInt.toString(16), "conversion failed");
        ...
        toStruct = await imbalanceInst.callDecodeTokenImbalanceData("0x" + startInt.toString(16));

        toInt = await imbalanceInst.callEncodeTokenImbalanceData(toStruct[0], toStruct[1], toStruct[2], toStruct[3]);
        toInt = new BigNumber(toInt);
        // console.log("Start Int2: " + startInt.toString(16));
        // console.log("To Int2: " + toInt.toString(16));
        assert.equal(startInt.toString(16), toInt.toString(16), "conversion failed");
    });
    ...
    it("should test can't init this contract with empty contracts (address 0).", async function () {
        ...
        try {
           recorder = await MockImbalanceRecorder.new('0x0000000000000000000000000000000000000000');
           assert(false, "throw was expected in line above.")
        } catch(e){
           assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
    ...
    it("should test encode imbalance data reverts on data overflow or underflow.", async function() {
        ...
        */
        ...
        try {
            await imbalanceInst.callEncodeTokenImbalanceData(pow_2_64_div2.sub(0), legalValue, legalValue, legalValue);
            assert(false, "throw was expected in line above.")
        } catch(e){
            assert(Helper.isRevertErrorMessage(e), "expected throw but got: " + e);
        }
        ...
        //see success
        await imbalanceInst.callEncodeTokenImbalanceData(pow_2_64_div2.sub(1), legalValue, legalValue, legalValue);
        ...
        await imbalanceInst.callEncodeTokenImbalanceData(neg_pow_2_64_div2.add(1), legalValue, legalValue, legalValue);
        ...
        try {
            let number = new BigNumber(2).mul(pow_2_64_div2);
            await imbalanceInst.callEncodeTokenImbalanceData(legalValue, legalValue, legalValue, "0x" + number.toString(16));
            assert(false, "throw was expected in line above.")
        } catch(e){
            assert(Helper.isRevertErrorMessage(e), "using" + pow_2_64 +  " expected throw but got: " + e);
        }
        let number = pow_2_64.sub(1);
        await imbalanceInst.callEncodeTokenImbalanceData(legalValue, legalValue, legalValue, "0x" + number.toString(16));
